...
public final class CameraUtils {
    ...
    public static List<Map<String, Integer>> getAvailableResolutions(Activity activity, String cameraName) throws CameraAccessException {
        CameraManager cameraManager = (CameraManager) activity.getSystemService(Context.CAMERA_SERVICE);
        CameraCharacteristics characteristics = cameraManager.getCameraCharacteristics(cameraName);
        StreamConfigurationMap streamConfigurationMap =
                characteristics.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
        Size[] cameraSizeList = streamConfigurationMap.getOutputSizes(SurfaceTexture.class);
        List<Map<String, Integer>> list = new ArrayList<>();
        if (cameraSizeList != null && cameraSizeList.length > 0) {
            for (Size size : cameraSizeList) {
                HashMap<String, Integer> maps = new HashMap<>();
                maps.put("width", size.getWidth());
                maps.put("height", size.getHeight());
                list.add(maps);
            }
