...
- (void)onReceiveMessage:(JMSGMessage *)message error:(NSError *)error{
    NSMutableDictionary *argument = [message messageToDictionary];
   
    NSString *targetType = argument[@"targetType"];
    if([targetType isEqualToString:@"group"]){
        NSMutableDictionary *target = argument[@"target"];
        NSString *gid = target[@"id"];
        if([target[@"name"]  isEqual: @""]){
        ///获取群组信息
        [JMSGGroup groupInfoWithGroupId:gid completionHandler:^(id resultObject,NSError *error){
                if(!error){
                    argument[@"target"] = [resultObject groupToDictionary];
                     JMLog(@"target中name为空时，ios端接收到的IM消息group %@",argument.descriptionInStringsFileFormat);
                    [self->_channel invokeMethod:@"onReceiveMessage" arguments: argument];
                }else{
                     JMLog(@"ios端接收到的IM消息group info error %@",argument.descriptionInStringsFileFormat);
                    [self->_channel invokeMethod:@"onReceiveMessage" arguments: argument];
                }
                    }];
        }else{
            JMLog(@"targe中name不为空时，im消息group %@",argument.descriptionInStringsFileFormat);
            ///当name不为空时
            [self->_channel invokeMethod:@"onReceiveMessage" arguments:argument];
        }
      
    }else{
         JMLog(@"ios端接收到的IM消息single %@",argument.descriptionInStringsFileFormat);
         [self->_channel invokeMethod:@"onReceiveMessage" arguments: argument];
    }
